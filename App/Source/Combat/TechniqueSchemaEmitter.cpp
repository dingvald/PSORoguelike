#include "Combat/TechniqueSchemaEmitter.h"

#include "Combat/TechniqueError.h"
#include "Combat/TechniqueLibraryFile.h" // kTechniqueLibraryVersion
...
rapidjson::Document BuildTechniqueJsonSchema(const TechniqueSchemaModel& model)
{
    ...
    version_enum.PushBack(kTechniqueLibraryVersion, alloc);
...
void ValidateTechniqueDocument(const rapidjson::Document& document, const TechniqueSchemaModel& model)
{
    ...
    rapidjson::Document schema_doc = BuildTechniqueJsonSchema(model);
    ...
    if (!document.Accept(validator))
    {
        ...
        throw TechniqueError(std::string("technique file failed schema validation: keyword '") +
