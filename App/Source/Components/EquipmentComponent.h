...
// weapon slot, the four ArmorSlot body locations, and a mag companion slot.
// Deliberately NOT meta-registered (entt::entity has no FieldKind mapping,
// so it can't go through ComponentSchemaRegistrar at all): this is
// runtime-only state, set by game code as items are equipped, never
// hand-authored in a prefab JSON or listed in the editor's Add Component
// picker. Same precedent as TweenComponent. Unpopulated until the debug
// mission launcher (later in Phase A) starts setting it.
