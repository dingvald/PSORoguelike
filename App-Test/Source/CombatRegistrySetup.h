...
// every component with its own AttachHandlers, plus SetAffixLibrary/
// SetStatusEffectLibrary -- so Action tests exercise the same
// BeforeAttackEvent/BeforeTechniqueCastEvent/BeforePhotonArtCastEvent
// contribution path production code uses, not a hand-rolled substitute.
// Call before any entity that could carry EquipmentComponent/TPComponent/
// StatusEffectComponent is created. affixes/status_effects must outlive
// registry's use of them (same contract as Registry::SetAffixLibrary).
// GetStatusEffectLibrary() is called unconditionally on every landed hit
// (see AttackAction/PhotonArtAction/TechniqueAction's
// MaybeApplyElementalStatus call) and by Shock's cancellation check, so
// every combat-action test needs this even when it exercises no status
// effect directly -- a missing SetStatusEffectLibrary() call asserts, per
