...
void TPComponent::AttachHandlers(entt::registry& registry, entt::entity entity)
{
    ...
    events.Subscribe<BeforeTechniqueCastEvent, TPComponent>(
        [](Entity actor, BeforeTechniqueCastEvent& event) { ContributeTechniqueTp(actor, event); });
    events.Subscribe<BeforePhotonArtCastEvent, TPComponent>(
        [](Entity actor, BeforePhotonArtCastEvent& event) { ContributePhotonArtTp(actor, event); });
}
