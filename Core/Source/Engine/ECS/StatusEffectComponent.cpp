...
void StatusEffectComponent::AttachHandlers(entt::registry& registry, entt::entity entity)
{
    ...
    events.Subscribe<BeforePhotonArtCastEvent, StatusEffectComponent>(
        [](Entity actor, BeforePhotonArtCastEvent& event) { CancelIfShocked(actor, event); });
    events.Subscribe<BeforeTechniqueCastEvent, StatusEffectComponent>(
        [](Entity actor, BeforeTechniqueCastEvent& event) { CancelIfShocked(actor, event); });
