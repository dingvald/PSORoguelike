...
#include "Engine/Dungeon/DungeonLibraryFile.h" // kDungeonLibraryVersion
...
rapidjson::Document BuildDungeonJsonSchema(const DungeonSchemaModel& model)
{
    ...
    version_enum.PushBack(kDungeonLibraryVersion, alloc);
...
void ValidateDungeonDocument(const rapidjson::Document& document, const DungeonSchemaModel& model)
{
    ...
    rapidjson::Document schema_doc = BuildDungeonJsonSchema(model);
    ...
    if (!document.Accept(validator))
    {
        ...
        throw DungeonError(std::string("dungeon file failed schema validation: keyword '") +
