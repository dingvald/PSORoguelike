...
// this avoids inventing one. element_id is a free-form NameId, not a fixed
// enum -- the GDD explicitly declines to commit to a fixed element roster
// (only analogizing to PSO's Foie/Barta/Zonde), same treatment as
// RaceComponent::race_id. Deliberately carries no drain_percent field (unlike
// PhotonArt): EffectFamily::Drain still type-checks here since the enum is
// shared, but TechniqueAction resolves it identically to Damage for lack of
// an amount to size a restore by.
struct Technique
{
    std::uint32_t id = 0;
