...
namespace {
    constexpr int kEntitySchemaVersion = 1; // mirrors PrefabEditorLayer.cpp/GameplayLayer.cpp's own copy
    ...
    // Mirrors PhotonArtEditorLayer.cpp/AffixEditorLayer.cpp's own IdToPath.
    ...
    // Entity prefabs (App/Assets/Data/Entities) have no "name" field of their
    // own (see player.json), so the picker's display label is just the
    // authored id -- mirrors PrefabEditorLayer::RefreshPrefabList's own scan,
    // tolerant of a missing/malformed Entities directory (leaves just the
    // placeholder option rather than failing the whole form).
    std::vector<std::pair<std::uint32_t, std::string>> ListEntityPrefabOptions()
    {
        ...
        try
        {
            for (const JsonDirectoryEntry& entry : LoadJsonDirectory(EditorFilepaths::EntitiesPath, kEntitySchemaVersion))
                options.emplace_back(entt::hashed_string::value(entry.id.c_str()), entry.id);
        }
        catch (const std::exception&)
        {
            // Left at just the placeholder -- see doc comment above.
        }
        return options;
    }
} // namespace
...
void DropTableEditorLayer::RefreshEditForm()
{
    ...
    if (Rml::Element* row = m_editor->GetElementById("field-id"))
        keep(fieldwidgets::BuildStringField(*row, "id", m_draft_id,
                                            [this](std::string v)
                                            {
                                                ...
                                                if (Rml::Element* title = m_editor->GetElementById("edit-title"))
                                                    title->SetInnerRML(
                                                        EscapeRml(m_draft_id.empty() ? std::string{"(new drop table)"}
                                                                                     : m_draft_id));
                                            }));
    ...
    if (Rml::Element* row = m_editor->GetElementById("field-rare-chance"))
        keep(fieldwidgets::BuildIntField(*row, "rare_chance_percent", m_draft.rare_chance_percent,
                                         [this](int v)
                                         {
                                             m_draft.rare_chance_percent = v;
                                             MarkDirty();
                                         }));

    if (Rml::Element* row = m_editor->GetElementById("field-boss-guaranteed"))
        keep(fieldwidgets::BuildBoolField(*row, "boss_guaranteed_rare", m_draft.boss_guaranteed_rare,
                                          [this](bool v)
                                          {
                                              m_draft.boss_guaranteed_rare = v;
                                              MarkDirty();
                                          }));
...
void DropTableEditorLayer::RefreshEntryRows(bool rare)
{
    if (!m_editor)
        return;
    fieldwidgets::Listeners& row_listeners = rare ? m_rare_row_listeners : m_common_row_listeners;
    row_listeners.clear();

    Rml::Element* list = m_editor->GetElementById(rare ? "rare-entry-list" : "common-entry-list");
    if (!list)
        return;

    std::vector<DropEntry>& entries = rare ? m_draft.rare_entries : m_draft.common_entries;

    const std::vector<std::string> content(entries.size(),
                                           "<div class=\"entry-item field-row\"></div>"
                                           "<div class=\"entry-weight field-row\"></div>"
                                           "<div class=\"entry-section field-row\"></div>");

    fieldwidgets::RowList result = fieldwidgets::BuildRowList(
        *list, content, "<div class=\"list-empty\">No entries configured.</div>",
        [this, rare](std::size_t index)
        {
            std::vector<DropEntry>& target = rare ? m_draft.rare_entries : m_draft.common_entries;
            if (index < target.size())
                target.erase(target.begin() + static_cast<std::ptrdiff_t>(index));
            MarkDirty();
            RefreshEntryRows(rare);
        },
        [this, rare](std::size_t from, std::size_t to)
        {
            m_pending_action = [this, rare, from, to]
            {
                fieldwidgets::MoveElement(rare ? m_draft.rare_entries : m_draft.common_entries, from, to);
                MarkDirty();
                RefreshEntryRows(rare);
            };
        });

    const std::vector<std::pair<std::uint32_t, std::string>> item_options = ListEntityPrefabOptions();

    for (std::size_t i = 0; i < result.rows.size() && i < entries.size(); ++i)
    {
        const std::size_t index = i;
        const DropEntry& entry = entries[i];

        if (Rml::Element* row = result.rows[i]->QuerySelector(".entry-item"))
            for (auto& listener : fieldwidgets::BuildIdEnumField(
                     *row, "item_prefab_id", item_options, entry.item_prefab_id,
                     [this, rare, index](std::uint32_t id)
                     {
                         std::vector<DropEntry>& target = rare ? m_draft.rare_entries : m_draft.common_entries;
                         if (index < target.size())
                             target[index].item_prefab_id = id;
                         MarkDirty();
                     }))
                row_listeners.push_back(std::move(listener));

        if (Rml::Element* row = result.rows[i]->QuerySelector(".entry-weight"))
            for (auto& listener : fieldwidgets::BuildIntField(
                     *row, "weight", entry.weight,
                     [this, rare, index](int v)
                     {
                         std::vector<DropEntry>& target = rare ? m_draft.rare_entries : m_draft.common_entries;
                         if (index < target.size())
                             target[index].weight = v;
                         MarkDirty();
                     }))
                row_listeners.push_back(std::move(listener));

        if (Rml::Element* row = result.rows[i]->QuerySelector(".entry-section"))
            for (auto& listener : fieldwidgets::BuildEnumField(
                     *row, "favored_section_id", EnumOptions<SectionId>(), EnumToString(entry.favored_section_id),
                     [this, rare, index](std::string v)
                     {
                         std::vector<DropEntry>& target = rare ? m_draft.rare_entries : m_draft.common_entries;
                         if (index < target.size())
                             target[index].favored_section_id = EnumFromString(v, SectionId::None);
                         MarkDirty();
                     }))
                row_listeners.push_back(std::move(listener));
    }
...
{
    // Drains a reorder requested by fieldwidgets::WireDragReorder (via
    // BuildRowList), if any -- see its doc comment for why this can't run
    // synchronously from the "dragdrop" handler itself. Mirrors
    // DungeonEditorLayer::OnRender's identical drain.
    if (m_pending_action)
    {
